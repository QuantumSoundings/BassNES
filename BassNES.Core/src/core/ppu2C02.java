...
public class ppu2C02 implements java.io.Serializable{
		...
		case 4:
			if(scanline>=240||(!dorender())){
				if((OAMADDR&0x03)==0x02)
					b&=0xe3;
				map.ppuwriteoam((0xff&OAMADDR), b);
				OAMADDR++;
			}
			else
				OAMADDR+=4;
	...
	public byte readRegister(int index){
		...
		case 2:
			...
			break;
		case 4:
			OPEN_BUS = map.ppureadoam((0xff&OAMADDR));
			if(dorender()&&pcycle<=65&&scanline<241)
				return (byte)0xff;
			else{
				return map.ppureadoam((0xff&OAMADDR));
			}
		case 7:
			if((v&0x3fff)<0x3f00){
	...
	public void getBG(){
		...
		case 0://name table
			...
			cyclepart=0;
			nametablebyte = (0xff&map.ppureadNT(0x2000|(v&0x0fff)))<<4;
		...
		case 3://tile low
			...
			ptablemap0 = (0xff&map.ppureadPT(nametablebyte));
			cyclepart=3;
			break;
		case 4:cyclepart=4;break;
		case 5://tile high
			ptablemap1 = (0xff&map.ppureadPT(nametablebyte+8));
			cyclepart=5;
			break;
	...
	private void drawpixel(){
		if(render||(v&0x3f00)!=0x3f00){
			...
			pixels[pixelnum++] = (PPUMASK_colorbits)|(0xff&map.ppuread(v));
	}
	boolean delayset;
	int pixelColor(){
		int backgroundcolor=0;
	...
	private void stage3(){
		int y = (0xff&map.ppureadoam((4*n+m)));
	...
	public void spriteEvaluationNew(){
		if(pcycle<65){
			...
			case 1: // first write of stage 1.
				int y = (0xff&map.ppu_oam[n*4]);//ppureadoam(4*n));
	...
	void loadSprites(){
		//if((0xff&oambuffer[4*oamBCounter])!=0xff){
			...
			if((attributes&0x40)!=0)
				spritebm[spritec] = (BitReverseTable256[(0xff&map.ppureadPT(tileindex))]<<8)|(BitReverseTable256[(0xff&map.ppureadPT(tileindex+8))]);
			else
				spritebm[spritec] = ((0xff&(map.ppureadPT(tileindex)))<<8)|(0xff&(map.ppureadPT(tileindex+8)));
			spritefetch=false;
			if(tileindex<0)tileindex*=-1;
			//spritehorizontal[spritec]=(attributes&0x40)!=0;
			//spritebm[spritec] = ((0xff&(map.ppuread(tileindex)))<<8)|(0xff&(map.ppuread(tileindex+8)));
			if(scanline>=0&&(0xff&oambuffer[4*oamBCounter])!=0xff)
				numsprites++;
			spritec++;
			oamBCounter++;
	...
	void loadExtraSprites(){
		for(int i = extraSpriteStart;i<63;i++){
			int y = (0xff&map.ppu_oam[i*4]);//ppureadoam(4*n));
			if(PPUCTRL_ss?inrange(y)<16:inrange(y)<8){
				...
				int tileindex;
				if(PPUCTRL_ss){//get index number for different sprite sizes
					int temp = (0xff&map.ppu_oam[i*4+1]);
					tileindex = (temp&1)*0x1000+(temp&0xfe)*16;
					if(y>=8)//correct it for 8x16 sprites
						tileindex+=0x10;
				}
				else{
					tileindex=(0xff&map.ppu_oam[i*4+1])<<4;
					tileindex+= PPUCTRL_spta?0x1000:0;
				}
				if((attributes&0x80)!=0){//handle vertically flipped sprites
					if(PPUCTRL_ss){//case for large sprites
				...
				if((attributes&0x40)!=0)
					...
					spritebm[spritec] = ((0xff&(map.ppureadPT(tileindex)))<<8)|(0xff&(map.ppureadPT(tileindex+8)));
				if(scanline>=0&&(0xff&map.ppu_oam[4*i])!=0xff)
					numsprites++;
				spritec++;
			}
		}
