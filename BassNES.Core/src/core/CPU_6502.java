...
public class CPU_6502 implements java.io.Serializable{
	...
	public Object[] getDebugInfo(){
		...
		/*System.out.println("PC:" + Integer.toHexString(program_counter) + " Current Instruction: " + inst_name[current_instruction&0xff]
				//+ " Instruction Hex: "+Integer.toHexString((0xff&current_instruction))
				+ " Instruction Cycle: "+instruction_cycle
				+" SP:"+ Integer.toHexString((0xff&stack_pointer))
				+" A:" + Integer.toHexString((0xff&accumulator))
				+" X:" + Integer.toHexString((0xff&x_index_register))
				+" Y:" + Integer.toHexString((0xff&y_index_register))
				+" T:" + Integer.toHexString((0xff&tempregister))
				...
				+" C:" +(CFlag?1:0));
				*/
		return new Object[] {program_counter, name, instruction_cycle,(0xff&stack_pointer),
				(0xff&accumulator),(0xff&x_index_register),(0xff&y_index_register),NFlag,VFlag,DFlag,IFlag,ZFlag,CFlag,
				irqs,nmiInterrupt
		};
	}
	...
	public int getCurrentInstruction(){
		return (0xff&current_instruction);
	}
	private void executeInstruction(){
		...
		switch((0xff&current_instruction)){
		case 0x00:brk();break;
	...
	private void indx_r(){
		...
		case 2:
			...
			instruction_cycle++;break;
		case 3:
			pointer = pointer+(0xff&x_index_register);
			instruction_cycle++;break;
		case 4:
			pointer&=0xff;
	...
	private void adc(){
		int sum = (0xff&accumulator) + (0xff&tempregister) + (CFlag?1:0);
		CFlag = sum > 0xff;
	...
	private void asr(){
		...
		accumulator= (byte)((0xff&accumulator)>>1);
		ZFlag = accumulator ==0; NFlag = accumulator<0;		
	}
	private void atx(){
	...
	private void axs(){
		...
		int result = (0xff&x_index_register);
		result &= (0xff&accumulator);
		CFlag = result>=(0xff&tempregister);
		result-= tempregister;
	...
	private void brk(){
		...
		case 2:
			...
			instruction_cycle++;break;
		case 3:
			map.cpuwrite((0xff&stack_pointer)+0x100, (byte)(program_counter>>8));
			stack_pointer--;
			instruction_cycle++;break;
		case 4:
			//if(!(doNMI&&nmi))
			//	nmihijack = false;
			map.cpuwrite((0xff&stack_pointer)+0x100, (byte)(program_counter&0xff));
			stack_pointer--;
			instruction_cycle++;
			pollInterrupts();
			if(nmiInterrupt){
		...
		case 5:
			BFlag = true;
			map.cpuwrite((0xff&stack_pointer)+0x100, buildFlags());
			BFlag = false;
			if(nmihijack){
	...
	private void cmp(){
		CFlag = (0xff&accumulator)>=(0xff&tempregister);
		ZFlag = accumulator==tempregister;
		NFlag = (((0xff&accumulator)-(0xff&tempregister))&0x80)!=0;
		//if((0xff&accumulator)>=(0xff&tempregister)) CFlag = true;else CFlag = false;
		//if(accumulator == tempregister) ZFlag = true;else ZFlag = false;
		//if((((0xff&accumulator)-(0xff&tempregister))&0x80)!=0) NFlag = true;else NFlag = false;
	}
	private void cpx(){
		CFlag = (0xff&x_index_register)>=(0xff&tempregister);
		ZFlag = x_index_register==tempregister;
		NFlag = (((0xff&x_index_register)-(0xff&tempregister))&0x80)!=0;
		//if((0xff&x_index_register)>=(0xff&tempregister)) CFlag = true;else CFlag = false;
		//if(x_index_register == tempregister) ZFlag = true;else ZFlag = false;
		//if((((0xff&x_index_register)-(0xff&tempregister))&0x80)!=0) NFlag = true;else NFlag = false;
	}
	private void cpy(){
		CFlag = (0xff&y_index_register)>=(0xff&tempregister);
		ZFlag = y_index_register==tempregister;
		NFlag = (((0xff&y_index_register)-(0xff&tempregister))&0x80)!=0;
		//if((0xff&y_index_register)>=(0xff&tempregister)) CFlag = true;else CFlag = false;
		//if(y_index_register == tempregister) ZFlag = true;else ZFlag = false;
		//if((((0xff&y_index_register)-(0xff&tempregister))&0x80)!=0) NFlag = true;else NFlag = false;
	}
	private void dcp(){
	...
	private void isb(){
		...
		int sum = (0xff&accumulator) - (0xff&tempregister) - (CFlag?0:1);
		CFlag = (sum>>8 ==0);
	...
	private void jsr(){
		...
		case 3:
			instruction_cycle++;break;
		case 4:
			map.cpuwrite((0xff&stack_pointer)+0x0100, (byte)(program_counter>>>8));
			stack_pointer--;
			instruction_cycle++;break;
		case 5:
			map.cpuwrite((0xff&stack_pointer)+0x0100, (byte) (program_counter & 0x00FF));
			stack_pointer--;
			instruction_cycle++;
			pollInterrupts();break;
		case 6:
	...
	private void lsr(){
		...
		tempregister=(byte) ((0xff&tempregister)>>>1);
		ZFlag = tempregister==0;NFlag = tempregister<0;
	}
	private void nmi(){
		switch(instruction_cycle){
	...
	private void pha(){
		...
		case 2:
			...
			pollInterrupts();break;
		case 3:
			map.cpuwrite((0xff&stack_pointer)+0x0100, accumulator);
			stack_pointer--;
			instruction_cycle = 1;break;
		}
	}
	private void php(){
		...
		case 3:
			BFlag = true;
			byte x =buildFlags();
			map.cpuwrite((0xff&stack_pointer)+0x0100,x);
			BFlag = false;
			stack_pointer--;
			instruction_cycle = 1;break;
		}
	...
	private void pla(){
		...
		case 3:
			...
			pollInterrupts();break;
		case 4:
			accumulator = map.cpuread((0xff&stack_pointer)+0x0100);
			ZFlag = accumulator==0;NFlag = accumulator<0;
			instruction_cycle = 1;break;
		}
	}
	private void plp(){
		...
		case 4:
			setFlags( map.cpuread((0xff&stack_pointer)+0x0100));
			instruction_cycle = 1;break;
		}
	}
	private void rla(){
	...
	private void rra(){
		...
		if(CFlag){
			CFlag = (tempregister&1)!=0;
			tempregister = (byte) (((0xff&tempregister)>>1) | 0x80);
		}
		else{
			CFlag = (tempregister&1)!=0;
			tempregister = (byte) ((0xff&tempregister)>>1);
	...
	private void rti(){
		...
		case 4:
			...
			instruction_cycle++;break;
		case 5:
			program_counter = map.cpureadu((0xff&stack_pointer)+0x0100);
			stack_pointer++;
			instruction_cycle++;
			pollInterrupts();break;
		case 6:
			program_counter = program_counter| (map.cpureadu((0xff&stack_pointer)+0x0100)<<8);
			instruction_cycle = 1;
			break;
		}
	}
	...
	private void shx(){
		...
		case 4:
			...
			break;
		case 5:
			int t =((0xff&x_index_register)&((address>>8)+1))&0xff;
			lowpc = address&0xff00;
			address+=(0xff&y_index_register);
	...
	private void shy(){
		...
		case 4:
			...
			break;
		case 5:
			int t =((0xff&y_index_register)&((address>>8)+1))&0xff;
			lowpc = address&0xff00;
			address+=(0xff&x_index_register);
			address = (address&0xff)|lowpc;
			map.cpuread((address&0xff)|lowpc);
