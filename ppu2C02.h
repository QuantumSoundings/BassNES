#include <cstdint>

//
// Created by Jordan on 9/29/2017.
//
#ifndef BASSNES_PPU2C02_H
#define BASSNES_PPU2C02_H
class Mapper;
class ppu2C02{
private:
    void incx();
    void incy();
    void genFrame();
    void drawpixel();
    int pixelColor();
    int inrange(int y);
    void stage2();
    void stage3();
public:
    Mapper* map;
    uint8_t BitReverseTable256[256] = {
            
    };
    uint8_t OAMADDR=0;
    uint8_t OPEN_BUS=0;
    int t=0;
    uint8_t PPUCTRL=0;
    uint8_t PPUCTRL_bna=0;
    bool PPUCTRL_vraminc=false;
    bool PPUCTRL_spta=false;
    bool PPUCTRL_bpta=false;
    bool PPUCTRL_ss=false;
    bool PPUCTRL_ms=false;
    bool PPUCTRL_genNmi=false;
    int scanline=0,finalscanline=241;
    int pcycle=0;
    uint8_t PPUMASK=0;
    bool PPUMASK_grey;
    bool PPUMASK_bl=0;
    int leftmask_b=0;
    bool PPUMASK_sl=false;
    int leftmask_s=0;
    bool PPUMASK_sb=false;
    bool PPUMASK_ss=false;
    bool render_b=false;
    int PPUMASK_colorbits=0;
    int fineX=0;
    bool even=false;
    int v=0;
    int tv=0;
    bool PPUSTATUS_vb=false;
    uint8_t PPUDATA_readbuffer=0;
    bool PPUSTATUS_so=false;
    bool PPUSTATUS_sz=false;
    int shiftreg16a=0;
    int shiftreg16b=0;
    int palettelatchold=0,palettelatchnew=0;
    int cyclepart=0;
    int nametablebyte=0,atablebyte=0,ptablemap0=0,ptablemap1=0;
    bool oddframe=false;
    bool doneFrame=false;
    bool oldspritezero=false,spritezero=false;
    bool prevrender=false;
    int oamBCounter=0,numsprites=0,spritec=0;
    bool palregion;
    bool spritefetch=false;

	int spritebm[64] = {};
	int spriteco[64] = {};
    bool spritepriority[64];
    bool spritehorizontal[64];
	uint8_t oambuffer[32] = {};
	int pixels[256 * 240] = {};
    int pixelnum=0;
    bool delayset;
	int spritepalette[64] = {};
	int n = 0, m = 0;
	int stage = 1;

    ppu2C02(Mapper* m);
    uint8_t readRegister(int index);
    void writeRegisters(int index, uint8_t b);
    bool dorender();
    void getBG();
    void doCycle();
    void render();
    void spriteEvaluationNew();
    void loadSprites();


};

#endif