...
public class MMC1 extends Mapper {
	...
	public void cartridgeWrite(int index, byte b){
		if(index>=0x8000&&index<=0xffff){
			if(b<0)
				...
				System.out.println("Preping the Shiftregister: From cpu inst:"+cpu.inst_name.get(cpu.current_instruction));
				shiftregister>>=1;
				...
				int x = (Byte.toUnsignedInt(b)&1)<<4;
				shiftregister|=x;
				System.out.println("EXECUTING THE SHIFT REGISTER at index "+Integer.toHexString(index)+" value:"+shiftregister);
				writeRegister(index);
				shiftregister = 0b10000;
			}
		}	
	...
	public void writeRegister(int index){
		if(index>=0x8000&&index<=0x9fff){// Control register
			...
			if(!CHR_ram)
				if(CHR_ROM_mode ==0){
					CHR_ROM[0]=CHRbanks[(shiftregister&0b11110)];
					CHR_ROM[1]=CHRbanks[(shiftregister&0b11110)+1];			
				}
				else
					CHR_ROM[0]=CHRbanks[shiftregister];
		}
		else if(index>=0xc000&&index<=0xdfff){// CHR bank 1 select
			//System.out.println("CHANGING UPPER CHR bank");
			if(!CHR_ram)
				if(CHR_ROM_mode==1)
					CHR_ROM[1]=CHRbanks[shiftregister];
		}
			...
			case 0: case 1:
				...
				break;
			case 2:
				PRG_ROM[1] = PRGbanks[(shiftregister&0b1111)&PRGbanks.length];break;
			case 3:
				PRG_ROM[0] = PRGbanks[(shiftregister&0b1111)&PRGbanks.length];break;
			}
		}
